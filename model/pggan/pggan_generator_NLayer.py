# PGGAN中间层输入，默认从第二层开始输入，第三层到第n层也是可以的
...
class PGGANGenerator(nn.Module):
    ...
    def forward(self, z, zf=True,label=None, lod=None, n = 1, wf = False, w = 0, **_unused_kwargs):
        if zf == True:
        ...
        if zf == True:
            x = z.view(z.shape[0], self.z_space_dim + self.label_size, 1, 1) #x:[n,512,1,1]

            if n>1:
                for res_log2 in range(self.init_res_log2, self.init_res_log2+(n-1)): # +0是第一层(n-1)， +1是第二层(n-2)，+2是第三层(n-3).....
        ...
        if wf == False:
          ...
          x = w
        # middle layer
        for res_log2 in range(self.init_res_log2+(n-1), self.final_res_log2 + 1): # +0是第一层(n-1)， +1是第二层(n-2)，+2是第三层(n-3).....
            current_lod = self.final_res_log2 - res_log2
            if lod < current_lod + 1:
                block_idx = res_log2 - self.init_res_log2
                x = self.__getattr__(f'layer{2 * block_idx}')(x)
